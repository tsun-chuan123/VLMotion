...
class YoloServoPerception():
    ...
    def apply(self, color_image, depth_image): 
        ...
        # Optional debugging displays (suppressible via env vars)
        display_received_images = bool(int(os.getenv('YOLO_DEBUG_RECEIVED', '0')))
        ...
        display_yolo_results_image = bool(int(os.getenv('YOLO_DEBUG_RESULTS', '0')))
        ...
        # Show task-relevant overlay window. Default off when running under GUI.
        display_task_relevant_image = bool(int(os.getenv('YOLO_DISPLAY', '0')))
        if display_task_relevant_image:
            ...
            if ball is not None:
                if self.masks_available: 
                    ...
                    # Build a binary mask for the polygon
                    mask_bin = np.zeros(task_relevant_image.shape[:2], dtype=np.uint8)
                    cv2.fillPoly(mask_bin, [poly], 255, lineType=cv2.LINE_AA)

                    # Apply a semi-transparent WHITE overlay only on the masked region
                    overlay_color = np.array([255, 255, 255], dtype=np.float32)
                    alpha = 0.40
                    region = task_relevant_image[mask_bin == 255].astype(np.float32)
                    blended = (1.0 - alpha) * region + alpha * overlay_color
                    task_relevant_image[mask_bin == 255] = np.clip(blended, 0, 255).astype(np.uint8)
                ...
                # Draw width and XYZ near the grasp point
