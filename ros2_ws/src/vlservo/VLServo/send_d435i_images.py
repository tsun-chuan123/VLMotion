...
import time
import numpy as np
from . import d435i_helpers as dh
...
def main(use_remote_computer, exposure):
    try: 
        ...
        pipeline, profile = dh.start_d435i(exposure)
        first_frame = True
        ...
        if use_remote_computer:
            address = 'tcp://*:' + str(yn.d435i_port)
        else:
            address = 'tcp://' + '127.0.0.1' + ':' + str(yn.d435i_port)
        ...
        while True:
            ...
            if first_frame:
                ...
                d435i_output = {'depth_camera_info' : depth_camera_info,
            ...
            d435i_output['color_image'] = color_image
            d435i_output['depth_image'] = depth_image
                
            socket.send_pyobj(d435i_output)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        prog='Send D435i Images',
        description='Send D435i images to local and remote processes.'
    )
    parser.add_argument('-r', '--remote', action='store_true', help='Use this argument when allowing a remote computer to receive D435i images. Prior to using this option, configure the network with the file forcesight_networking.py on the robot and the remote computer.')
    parser.add_argument('-e', '--exposure', action='store', type=str, default='low', help=f'Set the D435i exposure to {dh.exposure_keywords} or an integer in the range {dh.exposure_range}') 
