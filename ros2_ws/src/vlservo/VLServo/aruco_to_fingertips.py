...
def _default_urdf_path():
    """Resolve the default URDF path near this module or in package share.

    Tries the following in order:
    1) Next to this file
    2) Package share directory (if ament_index_python is available)
    """
    module_dir = os.path.dirname(os.path.abspath(__file__))
    candidate = os.path.join(module_dir, 'stretch_uncalibrated.urdf')
    if os.path.isfile(candidate):
        return candidate
    try:
        from ament_index_python.packages import get_package_share_directory
        share_dir = get_package_share_directory('vlservo')
        candidate2 = os.path.join(share_dir, 'stretch_uncalibrated.urdf')
        if os.path.isfile(candidate2):
            return candidate2
    except Exception:
        pass
    return candidate  # fallback (likely missing); caller will handle errors
...
class ArucoToFingertips:
    def __init__(self, urdf_filename=None, default_height_above_mounting_surface=None):
        ...
        if urdf_filename is None:
            self.urdf_filename = _default_urdf_path()
        else:
            self.urdf_filename = urdf_filename

        # Attempt to load URDF, but continue gracefully if missing
        try:
        ...
        for side in self.sides:
            ...
            if urdf is not None:
                fingertip_link = urdf.link_map.get(fingertip_link_name)
                aruco_link = urdf.link_map.get(aruco_link_name)
                if fingertip_link is not None and aruco_link is not None:
                ...
                # Default to identity if URDF not available
                T = np.eye(4)
